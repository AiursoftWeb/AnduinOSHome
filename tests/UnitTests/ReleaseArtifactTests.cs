...
public class ReleaseArtifactTests
{
    ...
    {
        var artifact = CreateArtifact("https://cf.anduinos.com/AnduinOS-2.0.3-amd64.sha256");

        Assert.IsTrue(artifact.HasValidUrls());
        Assert.AreEqual("AnduinOS-2.0.3-amd64.iso", artifact.IsoFileName);
    }
    ...
    private static ReleaseArtifact CreateArtifact(string checksumUrl)
    {
        ...
        {
            IsoUrl = "https://cf.anduinos.com/AnduinOS-2.0.3-amd64.iso",
            TorrentUrl = "https://cf.anduinos.com/AnduinOS-2.0.3-amd64.torrent",
